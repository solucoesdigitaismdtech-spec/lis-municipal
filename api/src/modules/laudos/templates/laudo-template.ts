 ...
 * laudo-template.ts
 *
 * Gera o HTML do laudo laboratorial no padrão profissional
 * (inspirado no WorkLab). Recebe os dados e devolve uma string
 * HTML completa que o PdfGeneratorService converte em PDF.
 *
 * Tudo aqui é personalizável: cores, logo, cabeçalho e rodapé
 * são montados a partir dos dados do laboratório.
 */
...
interface DadosLaudo {
  ...
  // Cor primária personalizável (padrão: teal)
  corPrimaria?: string;
}

/**
 * Converte o nome técnico da categoria em rótulo amigável.
 */
...
export function gerarHtmlLaudo(dados: DadosLaudo): string {
  const cor = dados.corPrimaria || '#0d9488';
  const idade = calcularIdade(dados.paciente.dataNascimento);

  // Logo: usa o do laboratório ou um placeholder genérico (SVG inline)
  ...
  // Monta as seções de exames agrupados por categoria
            ...
            <tbody>
              ${exame.valores
                .map((v) => {
                  ...
                  const seta =
                    v.situacao === 'ALTO' ? ' ↑' : v.situacao === 'BAIXO' ? ' ↓' : '';
                  return `
                  <tr>
                    <td>${v.campo}</td>
