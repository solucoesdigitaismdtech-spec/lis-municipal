...
exports.montarLaudoHtml = montarLaudoHtml;
function obterValor(valor) {
    if (valor && typeof valor === 'object' && 'valor' in valor)
        return String(valor.valor ?? '');
    return String(valor ?? '');
}
function obterUnidade(valor, ref) {
    if (valor && typeof valor === 'object' && valor.unidade)
        return valor.unidade;
    return ref?.unidade || '';
}
function obterReferencia(valor, ref) {
    if (valor && typeof valor === 'object' && valor.referencia)
        return valor.referencia;
    if (!ref)
        return '—';
    return ref.textoRef ? ref.textoRef : `${ref.minimo ?? '—'} a ${ref.maximo ?? '—'} ${ref.unidade || ''}`;
}
function foraDaFaixa(ref, valorStr) {
    const n = parseFloat(String(valorStr).replace(',', '.'));
    if (isNaN(n) || !ref)
        return false;
    if (ref.minimo != null && n < ref.minimo)
        return true;
    if (ref.maximo != null && n > ref.maximo)
        return true;
    return false;
}
function esc(s) {
...
function montarLaudoHtml(ordem) {
    ...
    const blocosExames = (ordem.itens || []).map((item) => {
        ...
        const linhas = Object.keys(valores).length === 0
            ...
            : Object.entries(valores).map(([campo, valor]) => {
                ...
                const uni = obterUnidade(valor, valoresRef.find((r) => r.campo === campo));
                const refObj = valoresRef.find((r) => r.campo === campo);
...
//# sourceMappingURL=laudo-html.template.js.map